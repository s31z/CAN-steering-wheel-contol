...
void MCP4151::setResistance(const uint32_t& res) {
    if (res >= 500 && res <= 100000) {
        int value = map(res, 0, 100000, 256, 0);
        Serial.print("MCP4151: Setting resistance to ");
        Serial.println(res);
        writeValue(value);
    } else {
        writeValue(0); // default value 100k
    }
}
